...
En [`wokwi/`](./wokwi/) esta el diagrama del circuito (con el firmware de micropython y el de C).

| Simulación publicada            | Link                                            |
| ------------------------------- | ----------------------------------------------- |
| MicroPython — contador con IRQ  | (https://wokwi.com/projects/474914432133117953) |
| MicroPython — señal por timer   | https://wokwi.com/projects/474916128628655105   |
| MicroPython — juego de reflejos | https://wokwi.com/projects/474914467060140033   |
| C/C++ — juego de reflejos       | (https://wokwi.com/projects/475442902587476993) |
...
Aquí no es un lujo, es el **único modo preciso de medir**: con polling el número reportado incluiría el retraso del propio bucle y dejaría de ser el tiempo de reacción del jugador para volverse una propiedad del programa.
...
La prueba del juego se corre en la simulación (wokwi.com, RP2040) y en la placa física (Pico 2 W, RP2350). El resto de códigos, únicamente en wokwi. El código fuente es el mismo, así que una discrepancia entre columnas apunta al hardware, no a la lógica.

### DO 01 — Botón (`button_irq_counter.py`)

| Prueba           | Esperado                       | Wokwi |
| ---------------- | ------------------------------ | ----- |
...
| Prueba                       | Esperado                                     | uPy (W) | C (W) | uPy (F) | C (F) |
| ---------------------------- | -------------------------------------------- | ------- | ----- | ------- | ----- |
...
| 4                  | 335     | 265   | 187    | 182   |
| 5                  | 342     | 293   | 159    | 101   |
| **Mejor**          | 299     | 259   | 159    | 101   |
| **Peor**           | 342     | 319   | 207    | 257   |
| **Promedio**       | 327     | 288   | 184    | 186.4 |
| **Salidas falsas** | 1       | 0     | 1      | 0     |
